...
exports.Streamsha.prototype = {
     ...
     * @param {string} filename  the The filename of the file to hash.
     * @param {function} callback The function to be called when (if) hashing is successful.
     * @param {string} [encoding] The encoding to use for the resulting hash, will be a buffer if left out.
     * @return this
     */
    hashFile: function(filename, callback, encoding) {
        var self = this;
        fs.readFile(filename, function(err, data) {
            if (err) throw err;
            var hash = self._hashData(data);
            callback(encoding ? hash.toString(encoding) : hash, self.hashedBuffer);
        });
        return this;
    },
